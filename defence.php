...
$max_purgeable_units = $row->c12;
$max_training=$row->b17+1;
...
if (!empty($_GET['cancel'])) {
	...
	$sk=sk($cancel);
	if ($row->{"i$sk"} >= $current_time) {
		$cost_a0 = $defence_units[$sk][0];
		$cost_a1 = $defence_units[$sk][1];
		$cost_a7 = $defence_units[$sk][2];
		$cost_a9 = $defence_units[$sk][3];
		$in_build--;
		$to_update .= ", `a0`=`a0`+$cost_a0,`a1`=`a1`+$cost_a1,`a7`=`a7`+$cost_a7,`a9`=`a9`+$cost_a9,`i$sk`='0'";
		$row->{"i$sk"}=0;
		$row->a0+=$cost_a0;
		$row->a1+=$cost_a1;
		$row->a7+=$cost_a7;
...
foreach ($_GET as $key=>$val) {
	$key = clean_post($key);
	$val = clean_post($val);
	if (isset($row->$key) and $in_build < $max_training) {
		$sk=sk($key);
		$amount=1;

		if (preg_match("@^e@",$key) and $sk <= $max_purgeable_units) {
			$cost_a0 = $defence_units[$sk][0];
			$cost_a1 = $defence_units[$sk][1];
			$cost_a7 = $defence_units[$sk][2];
			$cost_a9 = $defence_units[$sk][3];
			$timer_defence = timer_defence($sk,$amount);
			if ($sk <= 19 and $row->a0 >= $total_cost0+$cost_a0 and $row->a1 >= $total_cost1+$cost_a1 and $row->a7 >= $total_cost7+$cost_a7 and $row->a9 >= $total_cost9+$cost_a9 and $row->{"i$sk"} < $current_time) {
				$total_cost0+=$cost_a0;
				$total_cost1+=$cost_a1;
				$total_cost7+=$cost_a7;
				$total_cost9+=$cost_a9;
				$row->{"i$sk"} = $current_time+$timer_defence;
				$to_update .= ", `i$sk`=".$row->{"i$sk"};
				$in_build++;
			}
		}
	}
...
print '<table><tr><th colspan="2">Defensive Structures</th><th><a href="?';for($i=0;$i<=19;$i++) {print 'e'.$i.'&';}print '"><span id="xxs'.$xxs.'">'.number_format($in_build).'</span>/'.number_format($max_training).'</a></th></tr>';$xxs++;
foreach ($defence_units as $key=>$val) {
$def=defence_defence_power($key);
$tim=timer_defence($key,1);
print '<tr class="buildings"><td>'.imagine('d'.$key).'</td><td><b>'.$defence_units_names[$key].'</b>
<br>Units: '.number_format($row->{"e$key"}).'
<br>Requirements: '.$a_names[0].': '.number_format($defence_units[$key][0]).'
'.$a_names[1].': '.number_format($defence_units[$key][1]).' '.$a_names[7].': '.number_format($defence_units[$key][2]).'
'.$a_names[9].': '.number_format($defence_units[$key][3]).'
<br>Defence Power: '.number_format($def).'
<br>Construction Time: '.clockit($tim).'
<br>Garrison Unit: '.$military_units_names[$key];
if ($row->{"k$key"} <= $row->{"e$key"}) {
print '<br><span class="maxed">Insufficient Garrison</span>';
}
print '</td>
<td>';
if($key <= $max_purgeable_units){
	if ($row->{"i$key"} > $current_time) {
		$fresh_timer=$row->{"i$key"}-$current_time;
		if ($fresh_timer<$refresh_timer) {
			$refresh_timer=$fresh_timer;
		}
		print '<span id="xxt'.$xxt.'" class="timer">'.clockit($fresh_timer).'</span>';$xxt++;
	}else{
		if ($in_build < $max_training){
	print ($row->a0>=$defence_units[$key][0]&&$row->a1>=$defence_units[$key][1]&&$row->a7>=$defence_units[$key][2]&&$row->a9>=$defence_units[$key][3]?'<a href="?e'.$key.'">Build</a>':'Insufficient Resource');
	}else{
